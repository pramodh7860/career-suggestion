...
class DashboardManager {
  ...
  async init() {
    ...
    // Force update navigation to ensure logout button is shown
  ...
  async loadRecommendations() {
    try {
      ...
      if (response && response.ok) {
        const payload = await response.json()
        return (payload.data && payload.data.recommendations) || []
      }
    } catch (error) {
  ...
  async loadUserStats() {
    try {
      const response = await auth.apiRequest('/api/users/dashboard')
      if (response && response.ok) {
        const payload = await response.json()
        return (payload.data && payload.data.stats) || {}
      }
    } catch (error) {
  ...
  updateStats(stats) {
    document.getElementById('quizCount').textContent = stats.totalQuizzes || 0
    ...
    document.getElementById('progressDays').textContent = stats.completedQuizzes || 0
  }
    ...
    container.innerHTML = recentResults.map((result, index) => `
      <div class="result-item" data-result-id="${result._id}">
        ...
        <div class="result-score">${(() => {
          const avg = typeof result.averageScore === 'number' ? result.averageScore : null
          if (avg !== null) {
            return `${Math.round((avg <= 5 ? avg * 20 : avg) || 0)}%`
          }
          if (result.scores) {
    ...
    container.innerHTML = topRecommendations.map((career, index) => `
      <div class="recommendation-item" data-career-id="${career._id || ''}">
        ...
        <div class="recommendation-content">
          <h4>${career.careerTitle || career.title || 'Career'}</h4>
          <p>${career.reasoning || career.shortDescription || ''}</p>
        </div>
      </div>
    `).join('')
  ...
  renderSavedCareers() {
    ...
    container.innerHTML = this.savedCareers.map((career, index) => `
      <div class="saved-career-item" data-career-id="${career._id}">
        ...
        <h4>${career.title}</h4>
        <p>${career.savedAt ? new Date(career.savedAt).toLocaleDateString() : ''}</p>
      </div>
    `).join('')
  ...
  setupProgressChart() {
    ...
    try {
      ...
      console.error('Error setting up progress chart:', error)
    }
  }
